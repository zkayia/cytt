...
use crate::{
  celcat::models::Event,
  config::PUBLIC_PATH,
  templates::{GroupIcsTemplate, IndexHttpTemplate},
  utils::{
    date::dt_to_ics,
    fs::copy_recursively,
    image::{calc_text_size, fit_text_width, load_bold_font, load_font, rgb_from_text},
    num::{f32_i32, i32_u32, i32_usize, u32_i32, u64_u32},
  },
  CONFIG,
};
...
pub fn generate_ics(events: &Vec<Event>, path: PathBuf) -> anyhow::Result<()> {
  fs::write(
    path,
    GroupIcsTemplate {
      now: &dt_to_ics(&Local::now().naive_utc()),
      events,
    }
    .render()?,
  )?;
...
pub fn generate_png(events: &Vec<Event>, group_name: &str) -> anyhow::Result<()> {
  ...
  draw_hollow_rect_mut(
    &mut image,
    Rect::at(1, 1).of_size(1920 - 2, 1080 - 2),
    BORDER_COLOR,
  );
  ...
  for (i, pos) in (98..1920 - 2)
    .step_by(i32_usize(DAY_COLUMN_WIDTH)? + 2)
    .enumerate()
  {
  ...
  for (i, pos) in (0..).zip((54..1080).step_by(i32_usize(SCHEDULES_ROW_HEIGHT)? + 2)) {
    draw_filled_rect_mut(
      &mut image,
      Rect::at(0, pos).of_size(if i == 0 { 1920 } else { 98 }, 2),
      BORDER_COLOR,
    );
  ...
  for event in events {
    ...
    let event_rect_y = max(
      56 + u32_i32(max(start.hour() - 8, 0) * 60 + start.minute())? * (SCHEDULES_ROW_HEIGHT + 2)
        / 30,
      56 + 2,
    );
        ...
        Some(end) => max(
          ...
          min(
            u64_u32(
                ...
                .num_minutes()
                .unsigned_abs(),
            )? * i32_u32(SCHEDULES_ROW_HEIGHT + 2)?
              / 30,
    ...
    draw_filled_rect_mut(
      &mut image,
      event_rect,
      rgb_from_text(&event.celcat.background_color)?,
    );
    ...
    let event_info = [
      ...
      (
        ...
        &format!(
          ...
          event.subject.as_ref().unwrap_or(&"".to_owned())
        ),
      ),
