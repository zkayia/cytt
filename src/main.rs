...
use io::{generate_html, setup_public_dir};
...
async fn main() {
  logln!(
    "cytt {}{}",
    env!("CYTT_VERSION"),
    if option_env!("CYTT_IS_DOCKER").is_some() {
      " Docker"
    } else {
      ""
    }
  ...
  let cache_control =
    match HeaderValue::from_str(format!("max-age={}", CONFIG.calendar_fetch_interval).as_str()) {
      Ok(value) => value,
